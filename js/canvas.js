...
export class CircuitCanvas {
  ...
  setupEvents() {
    ...
    this.canvas.addEventListener('drop', (e) => {
      e.preventDefault();
      const type = e.dataTransfer.getData('text/plain') || e.dataTransfer.getData('text');
  ...
  handleMouseDown(e) {
    ...
    if (e.button === 0) { // 좌클릭
      ...
      const port = this.findPortAt(wx, wy, 16);

      // 부품의 포트 지점(8px 이내)이나 전선 포트를 직접 클릭한 경우 포트 드래그 모드
      if (port) {
        const portPos = port.getAbsolutePos();
        const distToPort = Math.hypot(portPos.x - wx, portPos.y - wy);

        if (port.parent.type === 'wire' || distToPort <= 8 || !comp) {
          this.saveHistory();
          this.activePort = port;
          this.linkedPorts = this.getConnectedPorts(port);
  ...
  handleMouseMove(e) {
    ...
    if (this.activePort) {
      ...
      if (wire && wire.type === 'wire') {
        ...
        // 연결된 상대 포트들(linkedPorts)도 같은 위치로 동시 이동
        const targetPorts = this.linkedPorts || (this.linkedPort ? [this.linkedPort] : []);
        targetPorts.forEach(lp => {
          const linkedWire = lp.parent;
          if (linkedWire && linkedWire.type === 'wire') {
            ...
            } else if (lp.id.startsWith('p_mid_')) {
              lp.absX = targetX;
              lp.absY = targetY;
    ...
    if (this.draggedComponent && this.draggedComponent.type !== 'wire') {
      ...
      // 부품이 이동할 때, 해당 부품의 각 Port에 연결되거나 맞닿은 전선의 끝점을 실시간 따라오게 갱신
      comp.ports.forEach(port => {
        ...
        connectedPorts.forEach(p => {
          if (p.parent && p.parent.type === 'wire') {
            const wire = p.parent;
            if (p.id === 'p_start') {
              wire.updatePoints(absPos.x, absPos.y, wire.endX, wire.endY);
            } else if (p.id === 'p_end') {
              wire.updatePoints(wire.startX, wire.startY, absPos.x, absPos.y);
            }
          }
        });
      });
  ...
  findPortAt(x, y, radius = 16) {
    ...
    for (const comp of this.components) {
      comp.ports.forEach(checkPort);
    }
    for (const wire of this.wires) {
      wire.ports.forEach(checkPort);
    }

    return bestPort;
  }

  getConnectedPorts(port) {
    if (!port) return [];
    const connected = new Set();
    const pos = port.getAbsolutePos();

    if (port.connectedTo) {
      connected.add(port.connectedTo);
    }
    ...
    allPorts.forEach(p => {
      if (p !== port && !connected.has(p)) {
        const pPos = p.getAbsolutePos();
        if (Math.hypot(pPos.x - pos.x, pPos.y - pos.y) <= 16) {
          connected.add(p);
        }
      }
    });
