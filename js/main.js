...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // pause control
  let paused = false;
  // UI pause/resume button (creates if missing)
  let btnPause = document.getElementById('btn-pause');
  if (!btnPause) {
    btnPause = document.createElement('button');
    btnPause.id = 'btn-pause';
    btnPause.textContent = 'Pause';
    btnPause.style.marginLeft = '8px';
    const speedSlider = document.getElementById('speed-slider');
    if (speedSlider && speedSlider.parentNode) {
      speedSlider.parentNode.appendChild(btnPause);
    } else {
  ...
  // 페이지 처음 로드시 기본 직렬 회로 템플릿 자동 로드
  loadTemplate('basic-circuit');
