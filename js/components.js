...
export class Lightbulb extends Component {
  ...
  draw(ctx) {
    ...
    const isGlowing = !this.burntOut && (this.brightness > 0 || Math.abs(this.current) > 1e-4);

    if (isGlowing) {
      const bVal = Math.max(0.3, this.brightness);
      ctx.shadowBlur = 20 + bVal * 35;
      ctx.shadowColor = `rgba(234, 179, 8, ${0.6 + bVal * 0.4})`;
    ...
    ctx.fillStyle = this.burntOut ? '#4b5563' : 
                     (isGlowing ? `rgba(253, 224, 71, ${0.4 + Math.min(1.0, this.brightness) * 0.6})` : '#334155');
    ctx.strokeStyle = this.burntOut ? '#dc2626' : 
                     (isGlowing ? `rgba(234, 179, 8, ${0.8 + Math.min(1.0, this.brightness) * 0.2})` : (isLit ? '#0f172a' : '#475569'));
    ctx.lineWidth = 4;
    ctx.beginPath();
...
export class CurrentMeter extends Component {
  constructor(id, x, y) {
    super(id, 'currentMeter', x, y);
    this.width = 120;
    this.height = 60;
    this.current = 0.0;
    this.label = "전류계";
  ...
  draw(ctx) {
    ...
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
    ctx.fillText(`${this.current.toFixed(2)}A`, 0, 0);

    ctx.fillStyle = '#94a3b8';
    ctx.font = 'bold 12px Outfit, sans-serif';
    ctx.fillText(this.id, 0, 30);
...
export class VoltageMeter extends Component {
  constructor(id, x, y) {
    super(id, {
      voltage: '전압계',
      currentMeter: '전류계',
    ...
    // 두 팔 포트 (연결되지 않아도 허용)
    this.ports = [
      new Port('p_plus', '+', this.width / 2 - 15, 0, this),
      new Port('p_minus', '-', -this.width / 2 + 15, 0, this)
    ];
  }
